...
class StratifiedKFoldTrainer:
    ...
    def _build_model(self, num_classes: int, multi_label: bool) -> torch.nn.Module:
        ...
        if task == "style":
            return StyleClassifier(num_classes=num_classes, **kwargs)
        if task == "instrument":
            return InstrumentClassifier(num_classes=num_classes, **kwargs)
        if task == "key_root":
            return KeyRootClassifier(num_classes=num_classes, **kwargs)
    ...
    def _stratify_label(self, movement_meta: Dict[str, Any]) -> str:
        ...
        if self.config.task == "style":
            value = labels.get("style", "unknown")
            return str(value).lower()
        ...
        values = labels.get("midi_instruments", [])
        if not values:
            return "unknown"
